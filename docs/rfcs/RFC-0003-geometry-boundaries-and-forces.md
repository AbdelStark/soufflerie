...
The inlet uses a regularized Zou/He velocity boundary with prescribed `u=U_ref*ramp(t)`, `v=0`, and density reconstructed from known populations. The outlet copies first-order non-equilibrium populations from the adjacent interior column with zero streamwise gradient and reconstructs equilibrium at the local density/velocity. The last `max(16, round(2*D_lu))` fluid columns form a sponge: collision relaxes toward the inlet equilibrium with strength increasing quadratically from `0` to `0.15`. The obstacle cannot enter the sponge.
