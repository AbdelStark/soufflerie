...
`ShapeParams` follows [`03-data-model.md#canonical-types`](../spec/03-data-model.md#canonical-types). The obstacle center is `(x_c=0.30*(nx-1), y_c=0.50*(ny-1))`. The unscaled reference major-axis diameter is `D_lu=ny/20`; `scale` produces major semi-axis `a=0.5*D_lu*scale` and minor semi-axis `b=a*aspect_ratio`. At cell-center coordinates, rotate into obstacle axes:
...
Preflight requires at least 12 cells across the scaled minor diameter; at least `2*D_lu` clearance to inlet, `4*D_lu` to outlet, and `1*D_lu` to either channel wall; and one-cell fluid connectivity from inlet to outlet. The public aspect-ratio range is `[0.5,1.0]`, so its minimum with `scale=0.75` is exactly 12 cells on the canonical `(ny=640,nx=512)` grid. Reduced solver smoke grids are internal numerical fixtures, not accepted public geometry cases; canonical-grid geometry preflight remains a CPU test. Any requested grid that cannot resolve the obstacle is rejected before solver-state allocation rather than rasterized inconsistently.

This `[0.5,1.0]` floor is the v0.1 resolution decision. Retaining the earlier `0.3` draft bound would produce a sub-12-cell minor diameter while relaxing the 12-cell gate would weaken the accepted boundary model. Authenticated L40S evidence rejected the original `12.5%` blockage domain (`Cd=1.75217`, `St=0.20065`), an `8.33%` domain (`Cd=1.60855`, `St=0.18571`), and a `6.25%` domain whose `Cd=1.56263` remained 0.65% above the fixed ceiling despite passing `St=0.18096`. The canonical grid therefore adds only transverse clearance while keeping `D_lu=32` and `nx=512`, so obstacle resolution and streamwise extents remain unchanged while blockage falls to `5%`. Its persisted `(ny=320,nx=256)` tensor is the exact isotropic `2x` reduction of the `(ny=640,nx=512)` solver domain; retaining the old 128-row tensor would have distorted the obstacle and was rejected. Thresholds were not changed. [Rejected 12.5% run](https://modal.com/apps/abdelhamid-bakhta/main/ap-6mQPv8brNL5NbF7Bd6UJuO), [rejected 8.33% run](https://modal.com/apps/abdelhamid-bakhta/main/ap-lmApE8MUB4bmTu1sepP4Ts), [rejected 6.25% run](https://modal.com/apps/abdelhamid-bakhta/main/ap-Ms94u31DL3atP8HuM9CwY0).

Boundary ownership order is: obstacle links, channel walls, inlet, outlet, then interior. The outer rows are inactive wall nodes and physical channel fluid spans rows `1..ny-2`. At each of the four corner-adjacent fluid cells, wall-crossing diagonal populations remain wall-owned; inlet or outlet reconstruction owns only the remaining streamwise incoming populations. Every destination population has one writer and the four mappings are fixture-backed. Top/bottom channel walls and obstacles use half-way link bounce-back: when pull streaming crosses a solid link, the destination population receives the opposite post-collision population from the fluid node. Obstacle links are enumerated by row-major fluid cell and then ascending D2Q9 direction. Force contribution on each link is accumulated in fp64 using fixed 256-link tiles and fixed tile order:
...
Channel walls are excluded from obstacle force because links derive only from the shared obstacle mask. Coefficients use the declared full `U_ref`, never the instantaneous ramp velocity. The correctness-first Warp path copies post-collision populations to the host for the same ordered reduction and uses no force atomics.
...
Force histories are sampled every 10 steps after `warmup_steps`. Mean Cd/Cl use samples in the right-closed interval `(last_sample_step - 4,000, last_sample_step]`. Strouhal requires at least eight resolved lift cycles and a non-degenerate lift signal. It subtracts the fp64 mean, applies a Hann window, uses an fp64 `rfft`, ignores zero frequency, and selects the strongest frequency in dimensionless `St in [0.05, 0.4]`. A three-bin parabola over log magnitude refines an interior peak by at most half a bin. Cycle count is the refined frequency times the observed span `last_step - first_step`. If prerequisites fail, `strouhal=None` with a typed diagnostic; the canonical cylinder gate requires a value.

Predicted-field Cd uses a rectangular control surface whose nearest side is at least eight output cells from the SDF zero contour and inside the sponge-free domain. The tightest rectangle one cell outside the `sdf < 8` output-cell clearance band's bounding box is selected; all four inclusive sides must remain in interior fluid and strictly before the sponge. With `p=cs2*rho`, fixed-order fp64 sums integrate pressure and convective streamwise momentum flux per unit span: `Fx = sum_left(p+rho*u^2) - sum_right(p+rho*u^2) + sum_bottom(rho*u*v) - sum_top(rho*u*v)`. Cases without a valid control surface are rejected at geometry preflight. This field estimate is a consistency signal, not the solver's momentum-exchange label.
